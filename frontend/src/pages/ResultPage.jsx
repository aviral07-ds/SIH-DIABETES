  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-6 gap-4">
        <div>
          <span className="text-xs font-bold text-sky-600 uppercase tracking-widest font-mono">STEP 3 OF 3: SCREENING RESULT</span>
          <h1 className="text-3xl font-extrabold text-slate-900 mt-1">Screening Result</h1>
          <p className="text-xs text-slate-500 mt-0.5">Instant point-of-care triaged evaluation for patient and community health counselor review</p>
        </div>

        <div className="bg-amber-50 border border-amber-200 px-4 py-2.5 rounded-2xl flex items-center space-x-2">
          <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping"></span>
          <div>
            <p className="text-[10px] font-bold text-amber-700 uppercase tracking-wider">CLINICAL TRIAGE LEVEL</p>
            <p className="text-sm font-extrabold text-amber-900">Follow-Up Recommended</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-amber-50/50 rounded-3xl p-6 sm:p-8 border border-amber-200/80 shadow-sm space-y-6 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <span className="text-xs font-bold text-amber-800 uppercase tracking-wider">AUTOMATED OBSERVATION</span>
                  <h2 className="text-3xl font-extrabold text-slate-900 leading-tight">Possible Signs Detected</h2>
                </div>
              </div>
            ...
            <div className="bg-white/80 rounded-2xl p-5 border border-amber-200 space-y-3">
              <div className="flex justify-between items-center text-xs font-bold">
                <span className="text-slate-500 uppercase tracking-wider">PRIMARY TRIAGE CLASSIFICATION</span>
                <span className="text-sky-700">AI Confidence: {data.metrics.confidence}%</span>
              </div>
            ...
            <div className="flex items-start space-x-3 text-xs text-slate-700">
              ...
              <p className="leading-relaxed">The AI system found signs that may indicate microvascular changes in the retinal blood vessels.</p>
            </div>

            {/* 4 Stat Pills */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
              <div className="bg-white p-3 rounded-xl border border-amber-200/80 text-xs">
                <p className="text-[10px] text-slate-400 font-medium">Eye Examined</p>
                <p className="font-bold text-slate-900 mt-0.5">{data.patient.eye}</p>
              </div>

              <div className="bg-white p-3 rounded-xl border border-amber-200/80 text-xs">
                <p className="text-[10px] text-slate-400 font-medium">Image Quality</p>
                <p className="font-bold text-emerald-700 mt-0.5">Optimal ({data.metrics.imageQuality}%)</p>
              </div>

              <div className="bg-white p-3 rounded-xl border border-amber-200/80 text-xs">
                <p className="text-[10px] text-slate-400 font-medium">Screening ID</p>
                <p className="font-mono font-bold text-slate-900 mt-0.5">#{data.screeningId}</p>
              </div>

              <div className="bg-white p-3 rounded-xl border border-amber-200/80 text-xs">
                <p className="text-[10px] text-slate-400 font-medium">Evaluation Time</p>
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="font-bold text-slate-900 text-sm">Why did the AI give this result?</h4>
                <p className="text-xs text-slate-500">Visual proof helps doctors and health workers quickly review the findings.</p>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-[10px] font-bold text-teal-700 uppercase tracking-widest">ACTION PLAN</span>
                <h3 className="font-extrabold text-slate-900 text-lg">Recommended Next Step</h3>
              </div>
            </div>

            <div className="p-4 rounded-2xl bg-sky-50/70 border border-sky-200 text-xs text-slate-800 leading-relaxed font-medium">
              Please consult a qualified eye-care professional or ophthalmologist for further evaluation and slit-lamp examination.
            </div>
            ...
            <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 flex items-start space-x-3 text-xs">
              ...
              <div>
                <p className="text-[10px] font-bold text-amber-800 uppercase tracking-wider">TARGET TIMEFRAME</p>
                <p className="font-bold text-amber-950 text-sm mt-0.5">Recommended visit within 3 to 4 weeks.</p>
              </div>
            </div>

            {/* Worker Checklist */}
            <div className="space-y-3 pt-2 border-t border-slate-100">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">WORKER CHECKLIST</p>
              <div className="space-y-2 text-xs text-slate-700 font-medium">
                <label className="flex items-center space-x-2">
                  <CheckSquare className="w-4 h-4 text-emerald-600" />
                  <span>Print or hand patient referral slip</span>
                </label>
                <label className="flex items-center space-x-2">
                  <CheckSquare className="w-4 h-4 text-emerald-600" />
                  <span>Share local district hospital eye-clinic contact</span>
                </label>
                <label className="flex items-center space-x-2">
                  <CheckSquare className="w-4 h-4 text-emerald-600" />
                  <span>Advise blood sugar and blood pressure recording</span>
                </label>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200">
              ...
              >
                ...
                <span>Print / Save Result Slip</span>
              </button>
              ...
              >
                ...
                <span>View Full Clinical ABDM Report</span>
              </button>
              ...
              >
                ...
                <span>Screen Another Person</span>
              </button>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-500 flex items-start space-x-3">
            ...
            <p><span className="font-semibold text-slate-700">Important:</span> This is an AI-assisted screening tool, not a medical diagnosis. A doctor's confirmation is always recommended.</p>
          </div>
